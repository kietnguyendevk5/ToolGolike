...
class AppCLI:
    ...
    def input_cookie(self):
        ...
        while True:
            ...
            if not line:
                break
            self.fb_accounts.append({"raw": line})
            i += 1  
        ...
        for acc in self.fb_accounts:
            ...
            try:
                ...
                if "success" in info:
                    ...
                    # =========================
                    # 1. LẤY PAGE TOKEN TỪ USER TOKEN
                    # =========================
                    page_token_map = {}
                    ...
                    try:
                        ...
                        for p in r.json().get("data", []):
                            ...
                            if pname and ptoken:
                                page_token_map[pname] = ptoken   # 🔥 map theo NAME

                    except Exception as e:
                        print("❌ Lỗi lấy page token:", e)

                    # =========================
                    # 2. LẤY PAGE LIST TỪ COOKIE
                    # =========================
                    pages = []
                    ...
                    for p in fb.getPages():
                        ...
                        pages.append({
                            ...
                            "name": pname,
                            "cookie": page_cookie,
                            "token": page_token_map.get(pname)  # 🔥 gán theo NAME
                        })
    ...
    def select_accounts(self):
        ...
        picks = input("Chọn (vd: 1,2,5 | all | all_acc | all_page): ").strip().lower()
        ...
        print(f"Đã chọn {len(self.selected)} acc/page")
    ...
    def build_table(self):
        ...
        for uid, d in self.table.items():
            ...
            content.append(f"Thất bại: {d['fail']}\n", style="bold")
            content.append(f"Proxy: {d.get('proxy', 'None')}\n", style="bold")
            content.append(f"Chế độ: {self.run_mode}\n", style="bold")
            content.append("Hành Động:\n", style="bold")
            content.append(status_text)

            # 🔥 đổi màu theo trạng thái
            if "ERROR" in d["status"] or "|" in d["status"]:
                color = "red"
            elif "+" in d["status"]:
                color = "green"
            else:
                ...
                try:
                    ...
                    for i in range(3, 0, -1):
                        self.table[fb_id]["status"] = (
                            f"[{current_sv}] | {reaction} (+{coin} xu) sau {i}s | {job_id}\n{link}"
                        )
                        time.sleep(1)
                    ...
                    if self.run_mode == "token":
                         # ❌ token chỉ làm reaction + chỉ ID
                        if "pfbid" in object_id:
                            self.table[fb_id]["status"] = f"[{current_sv}] | TOKEN không chạy pfbid → REPORT"
                            api.report_job(job_id, fb_id)
                            continue
                    ...
                    if not res or not res.get("success"):
                        ...
                        self.table[fb_id]["status"] = f"[{current_sv}] | FB FAIL → REPORT"
                        api.report_job(job_id, fb_id)
                        continue
    ...
    def delete_account(self):
        ...
        for i, acc in enumerate(self.fb_accounts, 1):
            print(f"{i}. {acc['name']} | {acc['uid']}")

        raw = input("Chọn acc muốn xóa (vd: 1,3,5): ")

        try:
            indexes = [int(x.strip()) for x in raw.split(",") if x.strip().isdigit()]
        except:
            print("Input lỗi")
            return
        ...
        for i in sorted(indexes, reverse=True):
            if 1 <= i <= len(self.fb_accounts):
                self.fb_accounts.pop(i - 1)

        # 🔥 QUAN TRỌNG: SAVE LẠI FILE
        self.save_data()
